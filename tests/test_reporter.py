...
class ReportWriterTests(unittest.TestCase):
    ...
    def test_html_and_xlsx_include_line_level_diff_details(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn('data-filter-state', html)
            self.assertIn('<span class="count-label">긴급</span>', html)
            self.assertIn('<span class="count-hint">즉시 확인</span>', html)
            ...
            self.assertIn("const filterState = document.querySelector(\"[data-filter-state]\");", html)
            self.assertIn("const filterLabels =", html)
            self.assertIn("상태만 표시 중입니다", html)
            ...
            self.assertIn("detail-head", html)
            self.assertIn("detail-meta", html)
            self.assertIn("상태: 변경됨 | 분류: interface | 변경 수: 1", html)
    ...
    def test_html_status_shortcuts_filter_all_severities_and_hide_until_selected(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("<section class='jump-list' aria-label='상태별 바로가기' data-jump-list hidden aria-hidden='true'>", html)
            self.assertIn('<section class="summary-list" aria-label="비교 요약" data-summary-list hidden aria-hidden="true">', html)
            self.assertIn('<section class="filter-state" data-filter-state>', html)
    ...
    def test_html_filter_regions_have_structured_hidden_attributes(self) -> None:
        ...
        filter_state = parser.first("section", **{"data-filter-state": None})
        self.assertIn("filter-state", filter_state.get("class", ""))
    ...
    def test_share_bundle_contains_redacted_reports_and_excludes_raw_outputs(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with ZipFile(paths["share_zip"]) as archive:
                ...
                self.assertIn("docs/USER_GUIDE.html", names)
                self.assertIn("docs/COMMAND_GUIDE.md", names)
                self.assertIn("docs/COMMAND_GUIDE.html", names)
                self.assertIn("docs/DEVELOPER_GUIDE_BEGINNER.md", names)
                self.assertIn("docs/DEVELOPER_GUIDE_BEGINNER.html", names)
                self.assertIn("docs/VERSION_HISTORY.md", names)
                self.assertIn("docs/VERSION_HISTORY.html", names)
                self.assertIn("docs/RELEASE_CHECKLIST.md", names)
                self.assertIn("docs/RELEASE_CHECKLIST.html", names)
                self.assertIn("docs/DIAGNOSTIC_MODE_GUIDE.md", names)
                self.assertIn("docs/DIAGNOSTIC_MODE_GUIDE.html", names)
                self.assertIn("docs/ERROR_CODE_CATALOG.md", names)
                self.assertIn("docs/ERROR_CODE_CATALOG.html", names)
