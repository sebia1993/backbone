...
- 정상 범위 `cpu_usage`와 `memory_usage` 출력이 기준/대상 스냅샷에서 동일하면 다시 `변경없음`으로 표시합니다.
- 정상 범위 CPU/Memory 출력이 실제로 달라진 경우는 `정보`로 표시합니다.
- CPU/Memory의 `긴급`/`주의` 임계치 판정은 비교 대상 스냅샷의 현재값 기준으로 유지합니다.
- 정상 범위 CPU/Memory 동일 출력이 `변경없음` 집계에 포함되는지 확인하는 회귀 테스트를 추가했습니다.
...
- CPU 50% 미만과 memory FreeRatio 40% 초과는 출력이 동일해도 `정보`로 표시했습니다.
...
```text
dist\backbone_state_tracker_v0.8.34_20260612_source.zip
dist\backbone_state_tracker_v0.8.34_20260612_source.zip.sha256.txt
dist\backbone_state_tracker_v0.8.34_20260612_windows_exe.zip
dist\backbone_state_tracker_v0.8.34_20260612_windows_exe.zip.sha256.txt
dist\backbone_state_tracker_v0.8.34_20260612_release_manifest.txt
dist\backbone_state_tracker_v0.8.34_20260612_verify_release_package.ps1
```
