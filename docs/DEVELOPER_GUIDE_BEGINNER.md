...
- HTML 리포트: 등급 카드는 상태 필터로 동작하며, `상태별 바로가기` 버튼과 요약 카드, 상세 블록을 같은 기준으로 표시합니다. 기본 보기에서는 `긴급`/`주의`만 보이고 `정보`/`변경없음`은 해당 등급을 선택해야 보입니다.
...
건강 상태를 의미하는 `No alarm`, `No fault`, `Normal`, `None`은 알람 키워드 오탐을 줄이기 위해 분류용 haystack에서 제외합니다. 단, `cpu_usage`, `memory_usage`, `power_status`의 임계치/State 판정은 별도 구조화 파서로 처리하므로 출력 변경 여부와 관계없이 위험 상태로 표시합니다.
