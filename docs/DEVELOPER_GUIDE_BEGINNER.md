...
- `core/workflow_wizard.py`: 상태 수집 화면의 작업 진행 마법사 단계 상태 계산입니다.
...
v0.8.14부터 `write_package_checksum()`은 `.sha256.txt`에 `Date stamp`를 기록하고, Python/PowerShell 패키지 검증기는 이 값이 ZIP 파일명 날짜와 일치하는지 확인합니다.
v0.8.15부터 GUI 첫 화면은 `상태 수집`이며, 대시보드 메뉴는 제거되었습니다. 작업 진행 마법사는 상태 수집 화면 상단에 있고, 수집 시작/입력 오류/busy 차단은 `작업 로그` 화면으로 이동합니다.
v0.8.16부터 HTML 리포트 상단 요약은 표가 아니라 라벨-값 카드 구조입니다. GUI 비교 화면의 지표는 `최근 변경 상세` 상단 요약 바로 표시되며, 수집 화면은 단계 선택 라디오 없이 `사용자 지정 단계명` 입력만 사용합니다. 빈 단계명은 `점검시간_YYYYMMDD_HHMM`으로 저장하되 내부 자동 비교 슬러그는 `pre_work`, `bb3_off`, `post_restore`를 유지해야 합니다.
v0.8.16부터 도움말 문서는 `EXE 옆 docs`, PyInstaller 내장 `docs`, 소스 `docs` 순서로 탐색합니다. EXE 빌드 스크립트는 `docs` 폴더를 `--add-data`로도 포함해야 합니다.
v0.8.17부터 GUI 첫 화면은 `장비 설정`이며, 좌측 메뉴 순서는 `장비 설정`, `상태 수집`, `비교 결과`, `작업 로그`입니다. 설정 화면의 `상태 수집으로 이동` 버튼은 단순 화면 이동이며, 저장이나 수집을 자동 실행하지 않아야 합니다.

## 5. UI 유지보수 기준

현재 GUI는 밝은 운영 콘솔 구조입니다.

- 좌측 메뉴: 장비 설정, 상태 수집, 비교 결과, 작업 로그
- 장비 설정: 접속 계정, 대상 장비, 장비 목록 저장, 상태 수집으로 이동
- 상단 상태 바: 현재 상태, 기준 스냅샷, 비교 대상
- 상태 수집: 작업 진행 마법사, 사용자 지정 단계명 입력, 설정 점검, 수집 시작
- 작업 진행 마법사: 장비 설정 확인부터 최종 확인까지 다음 액션 안내
- 비교 결과: 얇은 지표 요약 바, 최근 변경 상세 목록, 판단/라인/변경 내용, 선택한 변경 행의 운영 메모
...
8. `python .\tools\verify_release_package.py .\dist\<package>.zip --require-manifest`로 배포 ZIP을 검증합니다. 파일명, sidecar 버전/날짜, manifest의 버전/날짜/패키지 레코드, ZIP 내부 경로 안전성, ZIP/manifest 중복 엔트리 여부가 맞는지도 확인합니다.
...
빌드 스크립트는 ZIP 생성 후 `tools/verify_release_package.py`를 자동 실행하므로 필수 파일 누락이나 `config/devices.yaml`, `outputs/`, `raw/` 같은 금지 경로가 포함되면 실패합니다. 또한 ZIP 내부 파일이 `backbone_state_tracker/` 루트 밖에 있거나 절대 경로, 드라이브 경로, `..` 이동 경로를 쓰거나 같은 경로가 중복되면 실패합니다. sidecar `Date stamp` 불일치와 `release_manifest.txt`의 같은 `Package` 레코드 중복도 실패합니다.
