...
| `cpu_usage` | `display cpu-usage` | CPU 상태 확인 | 5초/1분/5분 값 중 70% 이상은 긴급, 50~69%는 주의, 모두 50% 미만은 정상입니다. 정상 범위에서 출력이 달라진 경우 정보입니다. |
| `memory_usage` | `display memory` | 메모리 상태 확인 | FreeRatio 30% 이하는 긴급, 31~40%는 주의, 40% 초과는 정상입니다. 정상 범위에서 출력이 달라진 경우 정보입니다. |
...
| 정보 | 정상 범위 CPU/memory 출력 변화, 접속 복구 또는 긴급/주의로 단정하지 않는 일반 변경 |
| 변경없음 | 정상 범위 CPU/memory를 포함해 의미 있는 변경이 없는 항목 |
