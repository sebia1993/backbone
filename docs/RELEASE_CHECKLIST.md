...
- `.sha256.txt`의 `Version`과 `Date stamp`가 ZIP 파일명에 포함된 버전/날짜와 일치해야 합니다.
...
```text
backbone_state_tracker_v0.8.17_YYYYMMDD_source.zip
backbone_state_tracker_v0.8.17_YYYYMMDD_source.zip.sha256.txt
backbone_state_tracker_v0.8.17_YYYYMMDD_windows_exe.zip
backbone_state_tracker_v0.8.17_YYYYMMDD_windows_exe.zip.sha256.txt
backbone_state_tracker_v0.8.17_YYYYMMDD_release_manifest.txt
backbone_state_tracker_v0.8.17_YYYYMMDD_verify_release_package.ps1
CURRENT_RELEASE.txt
```
...
검증기는 ZIP 파일명 버전/날짜, `.sha256.txt`의 대상 ZIP 이름과 `Version`, `Date stamp`, `*_release_manifest.txt`의 `Version`, `Date stamp`, 해당 패키지 `Size`/`SHA256` 레코드, ZIP 내부 경로 안전성, ZIP/manifest 중복 엔트리 여부도 함께 비교합니다.
...
- `version mismatch` 또는 `date mismatch`가 나오면 ZIP, `.sha256.txt`, `release_manifest.txt` 중 서로 다른 버전/날짜 파일이 섞인 것입니다.
