...
- `정보`: 접속 복구, 정상 범위의 `cpu_usage`/`memory_usage` 출력 변화, 또는 긴급/주의 키워드가 없는 일반 출력 변화입니다.
- `변경없음`: 정상 범위의 `cpu_usage`/`memory_usage`를 포함해 의미 있는 변경이 감지되지 않은 항목입니다. HTML 리포트에서는 해당 등급을 선택한 뒤 필요 시 접힌 상세를 펼쳐 확인합니다.
