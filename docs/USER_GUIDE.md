...
- 각 점검 명령의 의미와 정상/주의 포인트는 앱의 `도움말 > 점검 명령어 가이드 열기`, 상태 수집 화면의 `명령어 가이드 열기`, 또는 `docs/COMMAND_GUIDE.md/html`에서 확인합니다. 사용자 가이드와 버전 변경내역도 도움말 메뉴에서 열 수 있습니다.
...
- HTML 리포트는 상단 요약을 라벨-값 카드로 보여주고, 변경 내용을 `기존값 → 신규값` 형태로 한 줄에 표시합니다.
...
- `긴급`, `주의`, `정보`, `변경없음` 지표는 최근 변경 상세 상단의 얇은 요약 바로 표시됩니다.
...
- HTML 리포트는 라벨-값 요약 카드, 명령별 변경 상세 테이블, 원본 unified diff를 함께 제공합니다.
...
- 요약 카드의 `등급`, `상태`, `장비`, `명령`, `분류`, `변경 수`, `첫 변경`, `요약`은 각 값 옆에 라벨이 붙어 있어 행 일부만 보여도 의미를 확인할 수 있습니다.
...
- v0.8.14부터는 `.sha256.txt`의 `Date stamp`도 ZIP 파일명 날짜와 일치하는지 확인합니다.
