...
| CycloneDX SBOM | ✅ GitHub Actions | app/repository/version/source commit의 결정적 UUIDv5 `serialNumber`와 runtime lock 20개 전체 name/version 검증 |
...
PR Validation에서는 hash lock으로 의존성을 설치한 Windows runner에서 회귀·smoke·합성 진단·Ruff·Bandit·의존성 감사를 실행합니다. 이어 통합 ZIP과 재현 가능한 CycloneDX 1.6 SBOM을 만들고 app/repository/version/source commit으로 결정적 UUIDv5 `serialNumber`를 기록합니다. `tools/verify_release_assets.py`는 같은 네 입력으로 예상 UUIDv5를 다시 계산하면서 ZIP 구조, SHA-256, manifest source commit, SBOM 고정 버전도 확인합니다.
