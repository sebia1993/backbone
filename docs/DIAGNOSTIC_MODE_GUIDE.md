...
진단 모드는 실제 장비 원본 로그를 외부로 보내지 않고 문제 원인을 추적하기 위한 기능입니다. 기본 자체 점검은 장비에 접속하지 않고 앱 리소스, 마스킹 정책, 안전 리포트 생성 흐름만 확인합니다.
...
외부 개발 PC에서는 실제 장비 없이 모의 서버로 접속 흐름을 검증합니다. 모의 장비 프로파일은 실제 장비 출력 복사본이 아니라 합성 응답만 사용합니다.
...
수집기 통합 검증은 `127.0.0.1`과 모의 서버 포트를 대상 장비로 지정해 수행합니다. 이때 장비명은 `mock-device-a` 같은 합성 이름을 사용합니다.
...
```text
백본 상태 추적기 v0.8.57 진단 티켓
원본 로그 포함=false

이벤트:
- BST-CON-301 긴급 단계=network 상태=failed 장비=DEV-001 요약=TCP 연결 시간이 초과됐습니다.
```
